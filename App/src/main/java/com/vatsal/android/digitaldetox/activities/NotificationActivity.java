...
public class NotificationActivity extends AppCompatActivity {
     ...
     */
    public ArrayList<UsageStats> getStats(UsageStatsManager statsManager, String interval){
...
//        Log.d("stats","app=" + stats.size());
        ...
        ArrayList<UsageStats> final_stats = new ArrayList<UsageStats>();
        final_stats.add(stats.get(0));
        for(int i=0;i< stats.size();i++){
            if(stats.get(i).getLastTimeStamp() > (System.currentTimeMillis() - (7* 24 * 60 * 60 * 1000))){
                if(!final_stats.contains(stats.get(i))){
                    final_stats.add(stats.get(i));
                }
            }
        }
        Log.d("stats","app=" + final_stats.size());
        Log.d("stats","stats=" + stats.get(0).getLastTimeForegroundServiceUsed());
        return final_stats;
    }
